...
try{
  ...
  const browserFresh=label=>async()=>{const page=await makePage();try{return await page.evaluate(async ({label,data,options})=>{const a=await import(`/${label}/dist/index.js`);return a.probe(new Uint8Array(data),options);},{label,data:Array.from(bytes),options});}finally{await page.close();}};
  await pair('browser-page-wasm-initialization',{candidate:browserFresh('candidate'),previous:browserFresh('previous')});
  const page=await makePage();
  const call=(label,workerMode)=>page.evaluate(async ({label,data,options,workerMode})=>{if(workerMode){const {createDeckProbeWorker}=await import(`/${label}/dist/worker.js`);const w=createDeckProbeWorker();try{return await w.probe(new Uint8Array(data),options);}finally{w.terminate();}}const a=await import(`/${label}/dist/index.js`);return a.probe(new Uint8Array(data),options);},{label,data:Array.from(bytes),options,workerMode});
  ...
  const warmWorker=label=>page.evaluate(async ({label,data,options})=>globalThis.__acceptanceWorkers[label].probe(new Uint8Array(data),options),{label,data:Array.from(bytes),options});
